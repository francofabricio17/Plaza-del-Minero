// ====================================================
// MODELO 3D Y ESCENA THREE.JS - PLAZA PRINCIPAL
// ====================================================
...
var controles;
var sound;
var ambientLight, hemisphereLight, directionalLight, directionalLight2;
var lucesPostes = [];
...
function init() {
    ...
    // ====================================================
    // GESTOR DE CARGAS OPTIMIZADO (LOADING MANAGER)
    // ====================================================
    ...
    manager.onLoad = function () {
        ...
        var iniciarDesdeCero = parametros.has("inicio");
        var historiaCompletada = sessionStorage.getItem("historiaCompletada");

        if (iniciarDesdeCero || historiaCompletada !== "true") {
            ...
            console.log("Historia ya completada. Entrando directamente a la plaza.");
        }
    };
    ...
    hemisphereLight = new THREE.HemisphereLight(0xffffff, 0x112211, 0.5);
    ...
    directionalLight2 = new THREE.DirectionalLight(0xaaccff, 0.3);
    ...
    // Helper para carga de submodelos GLTF
    ...
    // 3. Postes de Alumbrado con Campo de Luz Local e Independiente
    cargarModeloSeguro("assets/poste.glb", function(modeloBase) {
        modeloBase.scale.set(8.5, 8.5, 8.5);

        // Geometría y material de la bombilla (bolita brillante)
        var geometriaEsfera = new THREE.SphereGeometry(0.8, 16, 16);
        var materialEsfera = new THREE.MeshBasicMaterial({ color: 0xfffae6 });

        function crearPosteConLuz(x, y, z) {
            var posteClonado = modeloBase.clone();
            
            // PointLight con radio de acción corto y controlado (distance: 70, decay: 2.0 para que decaiga rápido)
            var luzFoco = new THREE.PointLight(0xffddaa, 3, 70, 2.0); 
            luzFoco.position.set(0, 20, 0); // Posición de la lámpara en la punta del poste

            // Esfera visual brillante simulando la bombilla encendida
            var bolitaLuz = new THREE.Mesh(geometriaEsfera, materialEsfera);
            bolitaLuz.position.set(0, 18, 0);

            var grupoPoste = new THREE.Group();
            grupoPoste.add(posteClonado);
            grupoPoste.add(luzFoco);
            grupoPoste.add(bolitaLuz);

            grupoPoste.position.set(x, y, z);
            scene.add(grupoPoste);

            lucesPostes.push(luzFoco);
        }

        crearPosteConLuz(-52, 7, 30);
        crearPosteConLuz(-52, 7, -5);
        crearPosteConLuz(-52, 7, -35);
        crearPosteConLuz(-52, 7, -65);
        crearPosteConLuz(-52, 7, -95);
        crearPosteConLuz(-25, 1.4, 79);
        crearPosteConLuz(-89.5, 7, -93.5);
        crearPosteConLuz(-89, 7, -117);
        crearPosteConLuz(-52, 7, -120);
        crearPosteConLuz(-130, 7, -150);
        crearPosteConLuz(-135, 7, -103);
        crearPosteConLuz(-52, 7, -150);
    });
    ...
    cargarModeloSeguro("assets/barandasplaza.glb", function(modeloBaranda) {
        ...
        var obj19 = modeloBaranda.clone(); obj19.scale.set(3, 9.5, 6); obj19.position.set(-127.5, 8.6, 30); obj19.rotation.set(0, Math.PI / 2, 0); scene.add(obj19);   
        var obj20 = modeloBaranda.clone(); obj20.scale.set(3, 9.5, 6); obj20.position.set(-127.5, 8.6, -45); obj20.rotation.set(0, Math.PI / 2, 0); scene.add(obj20);
        var obj21 = modeloBaranda.clone(); obj21.scale.set(6, 7, 7.5); obj21.position.set(-150, 13, 30); scene.add(obj21);
        var obj22 = modeloBaranda.clone(); obj22.scale.set(6, 7, 7.5); obj22.position.set(-150, 13, 2); scene.add(obj22);
        var obj103 = modeloBaranda.clone(); obj103.scale.set(6, 7, 5); obj103.position.set(-150, 13, -26); scene.add(obj103);

        // Agrupación de barandas complejas
        ...
        // Copia trasladada del grupo de barandas
    ...
    // 8. Elementos Urbanos y Estructuras
    ...
    cargarModeloSeguro("assets/muro.glb", function(modelomuro) {
        ...
        var pilar25 = modelomuro.clone(); pilar25.position.set(103, 1, -60); pilar25.rotation.y = Math.PI / 2; scene.add(pilar25);
        var pilar26 = modelomuro.clone(); pilar26.position.set(103, 1, -31); pilar26.rotation.y = Math.PI / 2; scene.add(pilar26);
        var pilar27 = modelomuro.clone(); pilar27.position.set(103, 1, 31); pilar27.rotation.y = Math.PI / 2; scene.add(pilar27);
        var pilar28 = modelomuro.clone(); pilar28.position.set(103, 1, 60); pilar28.rotation.y = Math.PI / 2; scene.add(pilar28);
        var pilar29 = modelomuro.clone(); pilar29.position.set(103, 1, 89); pilar29.rotation.y = Math.PI / 2; scene.add(pilar29);
        var pilar30 = modelomuro.clone(); pilar30.position.set(103, 1, 118); pilar30.rotation.y = Math.PI / 2; scene.add(pilar30);
        var pilar31 = modelomuro.clone(); pilar31.position.set(103.5, 1, 135); pilar31.rotation.y = Math.PI / 2; scene.add(pilar31);
    });

    cargarModeloSeguro("assets/puertasiglo.glb", function(modelopuerta) {
        modelopuerta.scale.set(35, 43, 25);
        var pilar22 = modelopuerta.clone(); pilar22.position.set(101.5, 2, 0.5); pilar22.rotation.y = -Math.PI / 2; scene.add(pilar22);
    });
...
// ====================================================
// EVENTOS DE VENTANA Y BUCLE DE ANIMACIÓN
// ====================================================
...
function animate() {
    ...
    // Proyección de marcadores 3D a coordenadas HTML 2D
    marcadores.forEach(function(marcador) {
...
// ====================================================
// CONTROL DE CLIMA Y ATMÓSFERA
// ====================================================

window.cambiarAtmosfera = function(modo) {
    console.log("Iniciando cambio de atmósfera a modo:", modo);
    
    var textureLoader = new THREE.TextureLoader();
    var texturaRuta = "";

    if (modo === 'dia') {
        texturaRuta = "imagenes/cielo.jpg";
        directionalLight.color.setHex(0xffffff);
        directionalLight.intensity = 1.0;
        hemisphereLight.color.setHex(0xffffff);
        hemisphereLight.groundColor.setHex(0x112211);
        ambientLight.color.setHex(0xffffff);
        ambientLight.intensity = 0.35;
        
        lucesPostes.forEach(function(luz) { luz.intensity = 0; });
        
    } else if (modo === 'atardecer' || modo === 'tarde') {
        texturaRuta = "imagenes/atardecer.jpg"; 
        directionalLight.color.setHex(0xff7722);
        directionalLight.intensity = 0.6; // Reducido para que no sature la luz general en el atardecer
        hemisphereLight.color.setHex(0xff9944);
        hemisphereLight.groundColor.setHex(0x3a1e12);
        ambientLight.color.setHex(0xff6622);
        ambientLight.intensity = 0.3;
        
        lucesPostes.forEach(function(luz) { luz.intensity = 2.0; }); // Intensidad moderada por cada poste
        
    } else if (modo === 'noche' || modo === 'nublado') {
        texturaRuta = "imagenes/noche.png";
        directionalLight.color.setHex(0x7799dd);
        directionalLight.intensity = 0.15;
        hemisphereLight.color.setHex(0x223366);
        hemisphereLight.groundColor.setHex(0x050a14);
        ambientLight.color.setHex(0x1a2544);
        ambientLight.intensity = 0.1;
        
        lucesPostes.forEach(function(luz) { luz.intensity = 3.5; });
    ...
    textureLoader.load(
        ...
        function (texture) {
            ...
            scene.background = texture;
            scene.environment = texture;
            console.log("Textura aplicada correctamente:", texturaRuta);
        },
        undefined,
        function (err) {
            console.error("ERROR 404: No se encontró la imagen:", texturaRuta, err);
            alert("No se encontró la imagen: " + texturaRuta);
        }
    );
};
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var btnNublado = document.getElementById("btn-nublado");
    var botonesClima = document.querySelectorAll(".btn-clima");

    function actualizarClaseActiva(botonSeleccionado) {
        botonesClima.forEach(function (btn) {
    ...
    if (btnDia) {
        btnDia.addEventListener("click", function () {
            actualizarClaseActiva(btnDia);
            window.cambiarAtmosfera("dia");
        });
    }

    if (btnTarde) {
        btnTarde.addEventListener("click", function () {
            actualizarClaseActiva(btnTarde);
            window.cambiarAtmosfera("atardecer");
        });
    }

    if (btnNublado) {
        btnNublado.addEventListener("click", function () {
            actualizarClaseActiva(btnNublado);
            window.cambiarAtmosfera("noche");
        });
    }
});
...
if (btnSalir) {
    btnSalir.addEventListener("click", function () {
        sessionStorage.removeItem("historiaCompletada");
        window.location.href = window.location.pathname + "?inicio=" + Date.now();
    });
}
